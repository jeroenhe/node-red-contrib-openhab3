...
module.exports = function (RED) {
    ...
    function OpenHABControllerNode(config) {
        ...
        function getInitialStateOfItems(config) {
            ...
            ax.get(url, options)
                ...
                if (response.data != null && response.data.length > 0) {
                    ...
                    node.log("Connection established. Emitting " +
                ...
                if (error.response) {
                    ...
                    var errorMessage = "Request error response status " +
                    ...
                    var errorMessage2 = "Request error: " +
        ...
        function startEventSource() {
            ...
            var eventsource_url = config.ohversion == "v3"
    ...
    RED.httpNode.get("/openhab3/items", function (req, res) {
        ...
        ax.get(url, options)
            ...
            .catch((error) => {
            if (error.response) {
                res.send("Request error http status: " +
                    error.response.status +
                    " for url " +
                    url);
            }
    ...
    function OpenHABIn2(config) {
        ...
        function getInitial() {
            ...
            openhabController.control(itemName, null, null, function (body) {
    ...
    function OpenHABOut2(config) {
        ...
        function saveValue(item, topic, payload) {
            openhabController.control(item, topic, payload, function () {
        ...
        this.on("input", function (msg) {
            ...
            var item = config.itemname && config.itemname.length != 0
                ? config.itemname
                : msg.item;
            var topic = config.topic && config.topic.length != 0 ? config.topic : msg.topic;
            var payload = config.payload && config.payload.length != 0
                ? config.payload
                : msg.payload;
            ...
            if (payload != null) {
                ...
                if (onlywhenchanged) {
                    //Actively get the initial item state
                    openhabController.control(item, null, null, function (body) {
                        //gather variables
                        var currentState = body.state;
                        if (currentState != null &&
                            payloadString != null &&
    ...
    function OpenHABEvents(config) {
        ...
        function startEventSource() {
            ...
            var eventsource_url = config2.ohversion == "v3"
